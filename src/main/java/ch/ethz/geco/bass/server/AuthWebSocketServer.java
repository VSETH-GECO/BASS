...
public abstract class AuthWebSocketServer extends AbstractWebSocket implements Runnable {
    ...
    protected List<AuthWebSocketServer.WebSocketWorker> decoders;

    private List<AuthWebSocket> iqueue;
     ...
     *
     * @see #AuthWebSocketServer(InetSocketAddress, int, List, Collection) more details here
     */
     ...
     */
    public AuthWebSocketServer( InetSocketAddress address ) {
     ...
     * @param decodercount
     *            The number of {@link AuthWebSocketServer.WebSocketWorker}s that will be used to process the incoming network data. By default this will be <code>Runtime.getRuntime().availableProcessors()</code>
     */
    public AuthWebSocketServer( InetSocketAddress address , int decodercount ) {
     ...
     *
     */
    public AuthWebSocketServer( InetSocketAddress address , List<Draft> drafts ) {
     ...
     */
    public AuthWebSocketServer( InetSocketAddress address , int decodercount , List<Draft> drafts ) {
     ...
     */
    public AuthWebSocketServer( InetSocketAddress address , int decodercount , List<Draft> drafts , Collection<WebSocket> connectionscontainer ) {
        ...
        for( int i = 0 ; i < decodercount ; i++ ) {
            AuthWebSocketServer.WebSocketWorker ex = new AuthWebSocketServer.WebSocketWorker();
            decoders.add( ex );
            ex.start();
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            socket.setReceiveBufferSize( AuthWebSocket.RCVBUF );
            ...
            if( decoders != null ) {
                for( AuthWebSocketServer.WebSocketWorker w : decoders ) {
                    w.interrupt();
                }
            }
            return;
            ...
            while ( !selectorthread.isInterrupted() ) {
                ...
                try {
                    ...
                    while ( i.hasNext() ) {
                        ...
                        if( key.isReadable() ) {
                            conn = (AuthWebSocket) key.attachment();
    ...
    public ByteBuffer createBuffer() {
        return ByteBuffer.allocate( AuthWebSocket.RCVBUF );
    }

    protected void queue( AuthWebSocket ws ) throws InterruptedException {
    ...
    public final void onWebsocketMessage( WebSocket conn, String message ) {
        onMessage((AuthWebSocket) conn, message );
    }
    ...
    public final void onWebsocketMessage( WebSocket conn, ByteBuffer blob ) {
        onMessage((AuthWebSocket) conn, blob );
    }

    @Override
    public final void onWebsocketOpen( WebSocket conn, Handshakedata handshake ) {
        if( addConnection( conn ) ) {
            onOpen((AuthWebSocket) conn, (ClientHandshake) handshake );
        }
    }
    ...
    public final void onWebsocketClose( WebSocket conn, int code, String reason, boolean remote ) {
        ...
        try {
            if( removeConnection( conn ) ) {
                onClose((AuthWebSocket) conn, code, reason, remote );
            }
        } finally {
            try {
                releaseBuffers( conn );
     ...
     * <p>
     * {@link #AuthWebSocketServer(InetSocketAddress, int, List, Collection)} allows to specify a collection which will be used to store current connections in.<br>
    ...
    public final void onWebsocketError( WebSocket conn, Exception ex ) {
        onError((AuthWebSocket) conn, ex );
    }

    @Override
    public final void onWriteDemand( WebSocket w ) {
        AuthWebSocket conn = (AuthWebSocket) w;
        try {
     ...
     *
     * @see #onOpen(AuthWebSocket, ClientHandshake)
    ...
    private Socket getSocket( WebSocket conn ) {
        AuthWebSocket impl = (AuthWebSocket) conn;
     ...
     */
    public abstract void onOpen( AuthWebSocket conn, ClientHandshake handshake );
    /**
     ...
     **/
    public abstract void onClose( AuthWebSocket conn, int code, String reason, boolean remote );
    /**
     * Callback for string messages received from the remote host
     *
     * @see #onMessage(AuthWebSocket, ByteBuffer)
     * @param conn The <tt>WebSocket</tt> instance this event is occuring on.
     * @param message The UTF-8 decoded message that was received.
     **/
    public abstract void onMessage( AuthWebSocket conn, String message );
    /**
     * Called when errors occurs. If an error causes the websocket connection to fail {@link #onClose(AuthWebSocket, int, String, boolean)} will be called additionally.<br>
     ...
     **/
    public abstract void onError( AuthWebSocket conn, Exception ex );
     ...
     **/
    public void onMessage( AuthWebSocket conn, ByteBuffer message ) {
    }
    ...
    public class WebSocketWorker extends Thread {
        ...
        private BlockingQueue<AuthWebSocket> iqueue;

        public WebSocketWorker() {
            iqueue = new LinkedBlockingQueue<AuthWebSocket>();
        ...
        public void put( AuthWebSocket ws ) throws InterruptedException {
            iqueue.put( ws );
        }

        @Override
        public void run() {
            AuthWebSocket ws = null;
            try {
                while ( true ) {
                    ByteBuffer buf;
                    ws = iqueue.take();
